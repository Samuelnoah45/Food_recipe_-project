...
<template>
  <button
    ...
    @click="$emit('click')"
    :class="props.class"
    class="items-center border border-transparent font-medium rounded-md"
  >
  ...
  <router-link v-else-if="props.to" :to="props.to" v-slot="{ navigate }">
    <button
      @click="navigate"
      :class="props.class"
      class="mx-4 items-center border border-transparent font-medium rounded-md"
    >
      <slot></slot>
    </button>
  </router-link>
...
.btn-primary {
  @apply bg-primary hover:bg-primary-dark   focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary text-white;
}
.btn-white {
