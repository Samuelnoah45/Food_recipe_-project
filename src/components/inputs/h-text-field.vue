...
<template>
  <div>
    ...
    <div class="relative rounded-md shadow-sm font-body" :class="props.class">
      <div v-if="props.leadingIcon" class="absolute inset-y-0 left-0 px-4 flex items-center pointer-events-none" :class="
      ...
      <!-- icon -->
      <div v-if="props.trailingIcon" class="absolute inset-y-0 right-0 pr-3 flex items-center hover:cursor-pointer"
        @click="togglePassword()">
        <component class="h-5 w-5" :class="[type === 'password' ? 'text-primary' : 'text-gray-400']"
          :is="props.trailingIcon"></component>
      </div>
    </div>
    <p v-if="!props.hideDetail" :visible="errorMessage" class="mt-2 text-sm text-red-600 font-body" id="email-error">
      {{ errorMessage }} &nbsp;
    </p>
  </div>
</template>
